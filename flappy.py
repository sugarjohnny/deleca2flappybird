...
def main():
    global HITMASKS, SCREEN, FPSCLOCK, FPS, bot, MODE, SCORES, EPISODE, MAX_SCORE, RESUME_ONCRASH, count
        ...
        if crashTest[0]:
            ...
            if MODE in [Mode.TRAIN_NOUI, Mode.TRAIN, Mode.TRAIN_REPLAY]:
                if count%10==0:
                    ...
                    with open('score_gameCNT.txt', 'a') as f:
                        f.write(f"{bot.gameCNT//10},{score}\n")
                        print(f"Game {bot.gameCNT//10}: {score}")
                count+=1
